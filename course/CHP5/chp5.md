...
> **前置知識：Mason 增益公式**——完整定義、名詞解釋與例題見**第 2 章 §七.3**，這裡只複習公式本體。它的用處是「看著訊號流圖一次寫出輸入到輸出的轉移函數」，不必逐步做方塊圖化簡。
...
| 名詞 | 意義 |
|---|---|
| **路徑增益** $P_k$ | 第 $k$ 條**前向路徑**（從輸入節點順著箭頭走到輸出節點、節點不重複）上所有分支增益的**乘積** |
| **迴路增益** $L_i$ | 一個封閉路徑（起點終點同一節點、中間不重複）上所有分支增益的乘積。$\Delta$ 第二項起只取**互不接觸**（沒有共用節點）的迴路組合，正負號逐項交替 |
| **餘因子** $\Delta_k$ | 把所有**碰到第 $k$ 條前向路徑**的迴路刪掉後、用剩下的迴路重算一次的 $\Delta$；若每個迴路都碰到該路徑，則 $\Delta_k=1$ |

> **⚠️ 用在取樣訊號流圖上要注意兩件事**：
>
> 1. **節點是星號變數**（$R^*$、$E_1^*$、$C^*$），分支增益是**星號轉移函數**（$G^*$、$\overline{GH}^*$）——因為這張圖是由步驟 4 星號化之後的方程式畫出來的。
> 2. **取樣器本身不在圖上**（它沒有轉移函數），它的輸出被當成**來源節點**，就是步驟 2、3 的做法。
>
> 另外，**負回授的 $-1$ 要吃進迴路增益**，所以 $L_1=-\overline{GH}^*$，代入得 $\Delta=1-(-\overline{GH}^*)=1+\overline{GH}^*$——這就是下面各例分母長成 $1+\overline{GH}^*$ 的來源。
