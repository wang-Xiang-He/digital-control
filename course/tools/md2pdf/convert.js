...
function findAllMarkdownFiles(dir) {
  const results = [];
  const skipDirs = new Set(['node_modules', 'tools', '.git']);
...
async function main() {
  ...
  if (args.length === 0 || args.includes('--all')) {
    targets = findAllMarkdownFiles(COURSE_ROOT);
    if (targets.length === 0) {
      console.error('在 course 目錄下找不到任何 .md 檔案。');
      process.exit(1);
    }
  } else {
  ...
  try {
    for (const target of targets) {
      const rel = path.relative(COURSE_ROOT, target);
      try {
        const pdfPath = await convertOne(md, browser, target);
        console.log(`  ✔ ${rel}  ->  ${path.relative(COURSE_ROOT, pdfPath)}`);
        ok += 1;
      } catch (err) {
